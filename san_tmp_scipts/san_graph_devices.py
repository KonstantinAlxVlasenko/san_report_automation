...
def link_group_description(connected_devices_df, group_type):
    
    # group_type = 'port_NPIV'
    # group_type = 'speed'
    link_description_column = 'Link_description_' + group_type
    group_columns = ['Fabric_name', 'Fabric_label', 'switchWwn', 'switchPair_id', 'Device_Host_Name', group_type, 'deviceType']
    ...
    device_links_df = dfop.merge_columns(device_links_df, summary_column=link_description_column, merge_columns=['Link_quantity', group_type], sep='x')
    ...
    # clean link description npiv
    if 'npiv' in group_type.lower():
...
link_description_speed_df = link_group_description(connected_devices_df, 'speed')
link_description_npiv_df = link_group_description(connected_devices_df, 'port_NPIV')    

# create link description for each switch->device 
# join and sport speed for switch->device (device ports connected to the same switch might have different speed )
...
# join speed and npiv 
device_links_df = pd.merge(link_description_speed_df, link_description_npiv_df, how='left', on=['Fabric_name', 'Fabric_label', 'switchWwn', 'switchPair_id', 'Device_Host_Name', 'deviceType'])
# create Link description column
device_links_df = dfop.merge_columns(device_links_df, summary_column='Link_description', merge_columns=['Link_description_speed', 'Link_description_port_NPIV'], sep=', ')
...
device_links_df['Link_description_sw_pair_level'] = device_links_df['switchPair_id_str'] + device_links_df['Fabric_label'] + ': ' + device_links_df['Link_description']
...
connected_devices_df['Link_description_fabric_name_level'] = connected_devices_df.groupby(by=['Fabric_name', 'deviceType', 'Device_Host_Name'])['Link_description_sw_pair_level'].transform('; '.join)
...
storage_on_grps_df = group_device_on_link_description(storage_above_threshold_df, san_graph_details_df)
...
def group_device_on_link_description(connected_devices_df, san_graph_details_df):
    ...
    # numbering groups
    connected_devices_df['Device_Group_number'] = connected_devices_df.groupby(by=['Fabric_name', 'deviceType', 'deviceSubtype_mod', 'Enclosure', 'Link_description_fabric_name_level'], dropna=False).ngroup().add(1)
    ...
    # remove duplicate device names if device is connected to multiple switshes in fabric label  and keep order for blade servers and sort for else
    ...
    device_shape_links_df = connected_devices_df.drop(columns=['Device_Host_Name', 'deviceSubtype', 'Device_Location', 'Device_Host_Name_slot', 'slot', 'slot_int', 'port_NPIV']).copy()
...
def group_device_on_name(connected_devices_df, san_graph_details_df):
    ...
    # some port maight be NPIV and some regular to one switch
    # link descripton already contains all ports
