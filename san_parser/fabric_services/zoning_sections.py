...
def regular_zoning_section_extract(san_cfg_lst, san_zone_lst, san_alias_lst, san_cfg_effective_lst, san_zone_effective_lst, 
                                    pattern_dct, principal_switch_lst, 
                                    line, file):
        ...
        if match_dct['cfg']:
            ...
            if cfg_line[1]:
                ...
                for member in members_lst:
                    san_cfg_lst.append([*principal_switch_lst, cfg_name, member])
            # if Effective configuration checked then 
            # add Effective and Defined configuration names to the table 
            if effective:
                san_cfg_effective_lst.append([*principal_switch_lst, cfg_name, ', '.join(defined_configs_set)])
            ...
            while not re.search(pattern_dct['zoning_switchcmd_end'], line):
                ...
                for member in members_lst:
                    san_cfg_lst.append([*principal_switch_lst, cfg_name, member.rstrip(';')])
                line = file.readline()
                if not line:
                    break
        # 'zone_match'
            ...
            if zone_line[1]:
                ...
                for member in member_lst:
                    ...
                    if not effective: 
                        san_zone_lst.append([*principal_switch_lst, zone_name, member])
                        sw_zone_lst.append([zone_name, member])
                    # for Effective configuration add zones to zone_effective_lst
                    elif effective:
                        san_zone_effective_lst.append([*principal_switch_lst, zone_name, member])
            line = file.readline()
            # zoning_switchcmd_end_comp separates different configs, zones and aliases
            while not re.search(pattern_dct['zoning_switchcmd_end'], line):
                members_lst = re.findall(pattern_dct['zones'], line)
                for member in members_lst:
                    # for Defined configuration add zones to zone_lst
                    if not effective:
                        san_zone_lst.append([*principal_switch_lst, zone_name, member.rstrip(';')])
                        sw_zone_lst.append([zone_name, member.rstrip(';')])
                    # for Effective configuration add zones to zone_effective_lst
                    elif effective:
                        san_zone_effective_lst.append([*principal_switch_lst, zone_name, member.rstrip(';')])
                line = file.readline()
                if not line:
                    break
        # 'alias_match'
            ...
            if alias_line[1]:
                ...
                for member in member_lst:
                    san_alias_lst.append([*principal_switch_lst, alias_name, member])
            line = file.readline()
            # zoning_switchcmd_end_comp separates different configs, zones and aliases
            while not re.search(pattern_dct['zoning_switchcmd_end'], line):
                member_lst = re.findall(pattern_dct['zones'], line)
                for member in member_lst:
                    san_alias_lst.append([*principal_switch_lst, alias_name, member.rstrip(';')])
                line = file.readline()
                if not line:
                    break
...
def peer_zoning_section_extract(san_peerzone_lst, san_peerzone_effective_lst, pattern_dct,
            ...
            while not re.search(pattern_dct['zoning_switchcmd_end'], line):
                ...
                if match_dct['peerzone_property']:
                    ...
                    if peerzone_effective:
                        san_peerzone_effective_lst.append(zonemember)
                    # for Defined configuration add member to peerzone_lst
                    else:
                        san_peerzone_lst.append(zonemember)
                    line = file.readline()
