...
def chassis_params_extract(all_config_data, report_creation_info_lst):
    ...
    *_, max_title = report_constant_lst
    ...
    data_lst = util.read_db(report_constant_lst, report_steps_dct, *data_names)
    
    # when any data from data_lst was not saved (file not found) or 
    # force extract flag is on then re-extract data from configuration files  
    force_run = util.verify_force_run(data_names, data_lst, report_steps_dct, max_title)
    ...
    if force_run:
        ...
        chassis_params, chassis_params_add, comp_keys, match_keys, comp_dct = util.data_extract_objects('chassis', max_title)
        ...
        for i, switch_config_data in enumerate(all_config_data):
            ...
            util.status_info('ok', max_title, len(info))

        # convert list to DataFrame
        chassis_params_fabric_df = util.list_to_dataframe(chassis_params_fabric_lst, max_title, sheet_title_import='chassis')
        # saving data to csv file
        data_lst = [chassis_params_fabric_df]
        # save_data(report_constant_lst, data_names, *data_lst)
        util.write_db(report_constant_lst, report_steps_dct, data_names, *data_lst)     

    # verify if loaded data is empty after first iteration and replace information string with empty list
    else:
        chassis_params_fabric_df = util.verify_data(report_constant_lst, data_names, *data_lst)
        data_lst = [chassis_params_fabric_df]

    # save data to excel file if it's required
    for data_name, data_frame in zip(data_names, data_lst):
        util.dataframe_to_report(data_frame, data_name, report_creation_info_lst)
