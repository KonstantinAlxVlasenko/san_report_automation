"""Module to generate edge device ports (storage, server, libraries) and it's connection to the switches shapes. 
...
def edge_device_shapes_compilation_init(portshow_aggregated_df, npv_ag_connected_devices_df, fcr_xd_proxydev_df, 
    ...
    """Main function to create device shape links tables"""
    ...
    data_names, analyzed_data_names = \
        dfop.list_from_dataframe(io_data_names_df, 'san_topology_device_out', 'san_topology_device_in')
    # module information
    ...
    if force_run:
        ...
        data_lst = [connected_devices_df, storage_shape_links_df, server_shape_links_df, 
                    san_graph_sw_pair_group_df, fabric_name_duplicated_sr, fabric_name_dev_sr]
        # writing data to sql
    ...
    # verify if loaded data is empty and replace information string with empty DataFrame
        ...
        _, storage_shape_links_df, server_shape_links_df, \
            san_graph_sw_pair_group_df, fabric_name_duplicated_sr, fabric_name_dev_sr, *_ = data_lst
...
def edge_device_shapes_compilation(portshow_aggregated_df, npv_ag_connected_devices_df, fcr_xd_proxydev_df, 
                                    switch_pair_df, san_graph_sw_pair_df, 
                                    san_graph_grid_df, pattern_dct, san_topology_constantants_sr):
    """Aggregated function to create edge device ports and it's connection to the switches shapes"""
    ...
    fabric_name_duplicated_sr = pd.Series(fabric_name_duplicated_lst, dtype='object', name='Fabric_name')
    fabric_name_dev_sr = pd.Series(fabric_name_dev_lst, dtype='object', name='Fabric_name')
