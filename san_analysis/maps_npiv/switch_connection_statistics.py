...
def connection_statistics(df, connected_dev_columns, tag):
    ...
    switch_conn_total_df = switch_conn_df.groupby(sw_columns).agg('sum', numeric_only=True).reset_index()
    ...
    # concatenate switch level with fabric_name and fabric_label summary
